...
from scrooge import signals as S
...
from scrooge.registry import Registry
from scrooge.serializer import Serializer
from scrooge.storage import (
    BlackHoleStorage,
    FileStorage,
    MemoryStorage,
...
class Scrooge(object):
    """
    Scrooge executes tasks by exposing function decorators that cause the function
    ...
    Example usage::
        ...
        @scrooge.periodic_task(crontab(minute='0', hour='2'))
        def nightly_report():
    ...
    def __init__(
        ...
        **storage_kwargs
        ...
        if invalid:
            warnings.warn(
                "the following Scrooge initialization arguments are no "
    ...
    def get_storage(self, **kwargs):
        if self.storage_class is None:
            warnings.warn(
                ...
                "scrooge, will default to RedisStorage."
            )
    ...
    def _get_timestamp(self):
        return utcnow() if self.utc else datetime.datetime.now()
...
class TaskWrapper(object):
    ...
    def __init__(
        ...
        **settings
        ...
        # Dynamically create task class and register with Scrooge instance.
        self.task_class = self.create_task(func, context, name, **settings)
        self.scrooge._registry.register(self.task_class)

    def unregister(self):
        return self.scrooge._registry.unregister(self.task_class)
    ...
    def is_revoked(self, timestamp=None, peek=True):
        return self.scrooge.is_revoked(self.task_class, timestamp, peek)

    def revoke(self, revoke_until=None, revoke_once=False):
        self.scrooge.revoke_all(self.task_class, revoke_until, revoke_once)

    def restore(self):
        return self.scrooge.restore_all(self.task_class)
    ...
    def schedule(
        ...
        eta = normalize_time(eta, delay, self.scrooge.utc)
        ...
        return self.scrooge.enqueue(task)

    def _apply(self, it):
        return [self.s(*(i if isinstance(i, tuple) else (i,))) for i in it]

    def map(self, it):
        return ResultGroup([self.scrooge.enqueue(t) for t in self._apply(it)])

    def __call__(self, *args, **kwargs):
        return self.scrooge.enqueue(self.s(*args, **kwargs))
...
class TaskLock(object):
    ...
    details see :py:meth:`Scrooge.lock_task`.
    """

    def __init__(self, scrooge, name):
        self._scrooge = scrooge
        self._name = name
        self._key = "%s.lock.%s" % (self._scrooge.name, self._name)
        self._scrooge._locks.add(self._key)

    def is_locked(self):
        return self._scrooge.storage.has_data_for_key(self._key)
    ...
    def __enter__(self):
        if not self._scrooge.put_if_empty(self._key, "1"):
            raise TaskLockedException("unable to acquire lock %s" % self._name)

    def __exit__(self, exc_type, exc_val, exc_tb):
        self._scrooge.delete(self._key)

    def clear(self):
        return self._scrooge.delete(self._key)
...
class Result(object):
    ...
    def __init__(self, scrooge, task):
        self.scrooge = scrooge
        self.task = task
    ...
    def _get(self, preserve=False):
        ...
        if self._result is EmptyData:
            res = self.scrooge.get_raw(task_id, peek=preserve)

            if res is not EmptyData:
                self._result = self.scrooge.serializer.deserialize(res)
                return self._result
            else:
                return res
        else:
            ...
            while self._result is EmptyData:
                if timeout and time_clock() - start >= timeout:
                    if revoke_on_timeout:
                        self.revoke()
                    raise ScroogeException("timed out waiting for result")
                if delay > max_delay:
                    delay = max_delay
    ...
    def is_revoked(self):
        return self.scrooge.is_revoked(self.task, peek=True)

    def revoke(self, revoke_once=True):
        self.scrooge.revoke(self.task, revoke_once=revoke_once)

    def restore(self):
        return self.scrooge.restore(self.task)

    def reschedule(
        self, eta=None, delay=None, expires=None, priority=None, preserve_pipeline=True
    ):
    ...
    */n = run every "n" times, i.e. hours='*/4' == 0, 4, 8, 12, 16, 20
    ...
    for date_str, value, acceptable in validation:
        ...
        for piece in value.split(","):
            ...
            # Older versions of Scrooge would, at this point, ignore the unmatched piece.
            if strict:
                raise ValueError("%s is not a valid input" % piece)
