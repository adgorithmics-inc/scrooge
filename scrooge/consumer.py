...
from scrooge.utils import time_clock
...
class BaseProcess(object):
    ...
    def create_logger(self):
        return logging.getLogger("scrooge.consumer.%s" % self.process_name)
...
class Worker(BaseProcess):
    ...
    def __init__(self, scrooge, default_delay, max_delay, backoff):
        self.delay = self.default_delay = default_delay
        self.max_delay = max_delay
        self.backoff = backoff
        super(Worker, self).__init__(scrooge)

    def initialize(self):
        for name, startup_hook in self.scrooge._startup.items():
    ...
    def shutdown(self):
        for name, shutdown_hook in self.scrooge._shutdown.items():
    ...
    def loop(self, now=None):
        ...
        try:
            ...
            if task is not None:
                ...
                try:
                    self.scrooge.execute(task, now)
                except Exception as exc:
                    self._logger.exception(
                        "Unhandled error during execution " "of task %s.", task.id
                    )
            elif not self.scrooge.storage.blocking:
                self.sleep()
...
class Scheduler(BaseProcess):
    ...
    def __init__(self, scrooge, interval, periodic):
        super(Scheduler, self).__init__(scrooge)
    ...
    def loop(self, now=None):
        ...
        try:
            task_list = self.scrooge.read_schedule(now)
        except Exception:
    ...
    def enqueue_periodic_tasks(self, now):
        ...
        for task in self.scrooge.read_periodic(now):
...
class Consumer(object):
    ...
    def __init__(
        ...
        self._logger = logging.getLogger("scrooge.consumer")
        if scrooge.immediate:
            self._logger.warning(
                "Consumer initialized with Scrooge instance "
    ...
    def flush_locks(self, *names):
        ...
        flushed = self.scrooge.flush_locks(*names)
        if flushed:
            self._logger.warning(
    ...
    def start(self):
        ...
        """
        if self.scrooge.immediate:
            raise ConfigurationError(
                "Consumer cannot be run with Scrooge instances where immediate "
                "is enabled. Please check your configuration and ensure that "
                '"scrooge.immediate = False".'
            )
        ...
        if self.worker_type == WORKER_GREENLET:
            if not monkey.is_module_patched("socket"):
                self._logger.warning(
                    "Gevent monkey-patch has not been applied"
        ...
        # Log startup message.
        self._logger.info(
            "Scrooge consumer started with %s %s, PID %s at %s",
            self.workers,
            self.worker_type,
            os.getpid(),
            self.scrooge._get_timestamp(),
        )
        ...
        for command in self.scrooge._registry._registry:
    ...
    def run(self):
        ...
        self.scrooge.notify_interrupted_tasks()
    ...
    def _handle_stop_signal(self, sig_num, frame):
        ...
        if self.worker_type == WORKER_GREENLET:
            ...
            def kill_workers():
                gevent.killall([t for _, t in self.worker_threads], KeyboardInterrupt)
