...
from scrooge.constants import WORKER_THREAD, WORKER_TYPES
...
class OptionParserHandler(object):
    def get_worker_options(self):
        return (
            ...
            option(
                ...
                dest="worker_type",
                help=(
                    "worker execution model (thread, greenlet, "
            ...
            option(
                ...
                metavar="SECONDS",
                help=(
                    "factor used to back-off polling interval when queue "
            ...
            option(
                ...
                help=(
                    "disable health check that monitors worker health, "
    ...
    def get_scheduler_options(self):
        return (
            ...
            option(
                ...
                action="store_false",
                dest="periodic",
                help="do NOT enqueue periodic tasks",
            ),
        )
    ...
    def get_logging_options(self):
        return (
            ...
            option(
                "quiet", action="store_false", dest="verbose", help="minimal logging"
            ),
            option(
                ("S", "simple"),
                action="store_true",
    ...
    def get_option_parser(self):
        parser = optparse.OptionParser(
            "Usage: %prog [options] " "path.to.scrooge_instance"
        )
        ...
        add_group(
            "Logging",
            "The following options pertain to logging.",
        ...
        add_group(
            "Workers",
            (
                "By default scrooge uses a single worker thread. To specify a "
        ...
        add_group(
            "Scheduler",
            (
                "By default Scrooge will run the scheduler once every second to check"
...
class ConsumerConfig(namedtuple("_ConsumerConfig", config_keys)):
    ...
    def validate(self):
        ...
        if not (0 < self.scheduler_interval <= 60):
            raise ValueError(
                "The scheduler must run at least once per "
                "minute, and at most once per second (1-60)."
            )
        if 60 % self.scheduler_interval != 0:
            raise ValueError(
                "The scheduler interval must be a factor of 60: "
    ...
    def setup_logger(self, logger=None):
        ...
        if self.simple_log:
            ...
            logformat = (
                "[%(asctime)s] %(levelname)s:%(name)s:" + worker + ":%(message)s"
            )
        if logger is None:
