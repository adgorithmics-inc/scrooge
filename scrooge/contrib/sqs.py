...
from scrooge.api import Scrooge
from scrooge.exceptions import ScroogeException
from scrooge.storage import BaseStorage, EmptyData
from scrooge.utils import utcnow
...
scrooge = SqsScrooge(
    name='scrooge',
    queue_name='scrooge_queue',
    bucket_name='scrooge.queue.results',
...
class SqsStorage(BaseStorage):
    ...
    def __init__(
        ...
        self.queue_name = queue_name or ("scrooge.%s" % queue_name)
        self.queue_settings = sqs_settings or {}
        self._sqs = boto3.resource("sqs")
        self._queue = None

        self.bucket_name = bucket_name or ("scrooge.%s" % bucket_name)
    ...
    def queue(self):
        if self._queue is None:
            try:
                self._queue = self._sqs.get_queue_by_name(QueueName=self.queue_name)
            except ClientError:
                self._queue = self._sqs.create_queue(
                    QueueName=self.queue_name, Attributes=self.queue_settings
                )
        return self._queue
        ...
        if self._bucket is None:
            ...
            if not bucket.creation_date:
                bucket = self._s3.create_bucket(
                    Bucket=self.bucket_name, **self.bucket_settings
                )
            self._bucket = bucket
        return self._bucket

    def enqueue(self, data, priority=None):
        self.queue.send_message(MessageBody=base64.b64encode(data).decode("ascii"))
    ...
    def add_to_schedule(self, data, ts, utc):
        ...
        if delay_seconds > 900:
            raise ScroogeException(
                "SQS does not support delays of greater than " "900 seconds."
            )
    ...
    def put_data(self, key, value, is_result=False):
        ...
        self.bucket.put_object(Body=value, Expires=expires, Key=key)
