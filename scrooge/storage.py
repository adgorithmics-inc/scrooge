...
from scrooge.utils import FileLock, text_type, to_timestamp
...
class BaseStorage(object):
    ...
    def __init__(self, name="scrooge", **storage_kwargs):
        self.name = name
    ...
    def add_to_schedule(self, data, ts, utc):
        ...
        :param bool utc: Whether scrooge is in UTC-mode or local mode.
        :return: No return value.
        """
        raise NotImplementedError
...
class RedisStorage(BaseStorage):
        ...
        **connection_params
        ...
        if Redis is None:
            raise ConfigurationError(
                '"redis" python module not found, cannot '
        ...
        self.queue_key = "scrooge.redis.%s" % self.name
        self.schedule_key = "scrooge.schedule.%s" % self.name
        self.result_key = "scrooge.results.%s" % self.name
        self.error_key = "scrooge.errors.%s" % self.name
    ...
    def dequeue(self):
        if self.blocking:
            try:
                return self.conn.brpop(self.queue_key, timeout=self.read_timeout)[1]
...
class RedisExpireStorage(RedisStorage):
    ...
    def __init__(self, name="scrooge", expire_time=86400, *args, **kwargs):
        super(RedisExpireStorage, self).__init__(name, *args, **kwargs)

        self._expire_time = expire_time

        self.result_prefix = rp = b"scrooge.r.%s." % self.name.encode("utf8")
...
class SqliteStorage(BaseSqlStorage):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # that SqliteScrooge may not strictly be a FIFO. If strict FIFO ordering
    ...
    def queue_size(self):
        return self.sql(
            "select count(id) from task where queue=?", (self.name,), results=True
        )[0][0]
    ...
    def schedule_size(self):
        return self.sql(
            "select count(id) from schedule where queue=?", (self.name,), results=True
        )[0][0]
    ...
    def peek_data(self, key):
        res = self.sql(
            "select value from kv where queue = ? and key = ?",
            (self.name, key),
            results=True,
        )
    ...
    def pop_data(self, key):
        with self.db(commit=True) as curs:
            curs.execute(
                "select value from kv where queue = ? and key = ?", (self.name, key)
            )
            result = curs.fetchone()
            if result is not None:
                curs.execute("delete from kv where queue=? and key=?", (self.name, key))
                if curs.rowcount == 1:
    ...
    def has_data_for_key(self, key):
        return bool(
            self.sql(
                "select 1 from kv where queue=? and key=?",
                (self.name, key),
                results=True,
            )
        )
    ...
    def result_store_size(self):
        return self.sql(
            "select count(*) from kv where queue=?", (self.name,), results=True
        )[0][0]

    def result_items(self):
        res = self.sql(
            "select key, value from kv where queue=?", (self.name,), results=True
        )
