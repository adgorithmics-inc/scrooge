...
from scrooge.constants import EmptyData
from scrooge.exceptions import (
    CancelExecution,
    ConfigurationError,
    RetryTask,
    TaskException,
...
from scrooge.serializer import SignedSerializer
...
class TestQueue(BaseTestCase):
    ...
    def test_workflow(self):
        ...
        self.assertEqual(len(self.scrooge), 1)  # One item in queue.
        task = self.scrooge.dequeue()
        self.assertEqual(len(self.scrooge), 0)  # No items in queue.
        self.assertEqual(self.scrooge.result_count(), 0)  # No results.
        ...
        self.assertEqual(self.scrooge.execute(task), 4)
        ...
        self.assertEqual(self.scrooge.result_count(), 0)
    ...
    def test_scheduling(self):
        ...
        self.assertEqual(self.scrooge.scheduled_count(), 0)

        task = self.scrooge.dequeue()
        self.assertFalse(self.scrooge.ready_to_run(task))
        value = self.scrooge.execute(task)  # Will not be run, will be scheduled.
        self.assertTrue(value is None)
        self.assertEqual(len(self.scrooge), 0)
        self.assertEqual(self.scrooge.scheduled_count(), 1)

        sched = self.scrooge.scheduled()
        ...
        self.assertTrue(self.scrooge.ready_to_run(task, timestamp))
        ...
        task2 = self.scrooge.dequeue()
        self.assertTrue(self.scrooge.ready_to_run(task2))
        self.assertEqual(self.scrooge.execute(task2), 5)
    ...
    def test_schedule_s(self):
        ...
        r1, r2 = self.scrooge.enqueue(task)
        self.execute_next()
        ...
        self.assertFalse(self.scrooge.ready_to_run(task))
        self.assertTrue(self.scrooge.ready_to_run(task, t10))

        t20 = datetime.datetime.now() + datetime.timedelta(seconds=20)
        oc = task.on_complete
        self.assertFalse(self.scrooge.ready_to_run(oc, t10))
        self.assertTrue(self.scrooge.ready_to_run(oc, t20))
        ...
        task = self.scrooge.dequeue()
        self.assertFalse(self.scrooge.ready_to_run(task))
        self.assertTrue(self.scrooge.ready_to_run(task, t20))
        self.assertEqual(self.scrooge.execute(task, t20), 6)
    ...
    def test_revoke_task(self):
        ...
        for r in (r1, r2, r3):
            ...
            self.assertTrue(self.scrooge.is_revoked(r))
            # We don't have the task class when using an ID so this doesn't
            # work.
            self.assertFalse(self.scrooge.is_revoked(r.task.id))

        # Task is discarded and not executed, due to being revoked.
        t1 = self.scrooge.dequeue()
        self.assertTrue(self.scrooge.execute(t1) is None)
        ...
        t3 = self.scrooge.dequeue()
        self.assertEqual(self.scrooge.execute(t3), 4)
    ...
    def test_revoke_task_instance(self):
        ...
        self.assertTrue(self.scrooge.is_revoked(r1))
        self.assertTrue(self.scrooge.is_revoked(r1.task.id))
        self.assertFalse(self.scrooge.is_revoked(r2))
        self.assertFalse(self.scrooge.is_revoked(r2.task.id))
        ...
        t2 = self.scrooge.dequeue()
        self.assertEqual(self.scrooge.execute(t2), 3)
    ...
    def test_revoke_task_instance_persistent(self):
        ...
        t2 = self.scrooge.dequeue()
        self.assertTrue(self.scrooge.execute(t2) is None)
        self.assertFalse(r2.is_revoked())  # No longer revoked.
        self.assertEqual(state, [])

        self.scrooge.enqueue(t1)
        self.scrooge.enqueue(t2)
        self.assertTrue(self.execute_next() is None)
        self.assertEqual(self.execute_next(), 3)
        self.assertEqual(r2.get(), 3)
        ...
        self.assertTrue(r1.is_revoked())
        self.assertFalse(r2.is_revoked())
        self.assertEqual(self.scrooge.result_count(), 1)  # t1's revoke id.
    ...
    def test_revoke_by_id(self):
        ...
        r1, r2, r3 = [task_a(i) for i in (1, 2, 3)]
        for r in (r1, r2, r3):
            self.scrooge.revoke_by_id(r.id)
            self.assertTrue(r.is_revoked())

        self.scrooge.restore_by_id(r2.id)  # Restore one instance.
    ...
    def test_revoke_until(self):
        ...
        for delta in (-second, zero, second, -second):
            task = self.scrooge.dequeue()
            self.scrooge.execute(task, timestamp + delta)
    ...
    def test_revoke_periodic(self):
        ...
        @self.scrooge.periodic_task(crontab(minute="0"))
        def task_p():
            state[0] = state[0] + 1
    ...
    def test_reschedule(self):
        ...
        self.assertEqual(len(self.scrooge), 2)
        ...
        self.assertEqual(self.scrooge.execute(task), 3)
    ...
    def test_reschedule_no_delay(self):
        ...
        @self.scrooge.task(context=True)
    ...
    def test_expires(self):
        ...
        @self.scrooge.task(context=True, expires=10)
        def task_r(task=None):
            state.append(task.id)
            return True

        @self.scrooge.task(context=True, expires=seconds(10))
        ...
        for exp_task in (task_r, task_a):
            ...
            self.scrooge.execute(task, seconds(12))
            self.assertEqual(state, [res.id])

            # Attempt to execute with execution timestamp at 5s.
            self.scrooge.execute(task, seconds(5))
            ...
            # Task is expired.
            self.scrooge.execute(task, seconds(63))
            self.assertEqual(state, [res.id, r2.id])

            # Run within the expiration time.
            self.scrooge.execute(task, seconds(50))
            ...
            self.scrooge.execute(task, seconds(31))
            self.assertEqual(state, [res.id, r2.id, r3.id])
            self.scrooge.execute(task, seconds(30))
    ...
    def test_scheduling_expires(self):
        ...
        (task,) = self.scrooge.read_schedule(timestamp=seconds(60))
        self.assertEqual(task.expires, 5)  # Expire time is present.
        self.scrooge.enqueue(task)

        task = self.scrooge.dequeue()  # Read our task back.
        task.eta = now  # Simulate the task being ready to run.
        self.assertEqual(self.scrooge.execute(task, seconds(1)), 4)
    ...
    def test_reschedule_expires(self):
        ...
        self.assertTrue(self.scrooge.execute(task, seconds(11)) is None)
        self.assertEqual(state, [])

        res3 = res2.reschedule(expires=5)
        task = self.scrooge.dequeue()
        self.assertTrue(seconds(5) < task.expires_resolved < seconds(7))
        self.assertEqual(self.scrooge.execute(task), True)
    ...
    def test_reschedule_priority(self):
        ...
        self.assertTrue(self.scrooge.execute(task))
    ...
    def test_retry(self):
        ...
        task_error = self.trap_exception(r)
        self.assertEqual(task_error.metadata["error"], "ExcCounter(1)")
        self.assertEqual(self.scrooge.result_count(), 1)  # No change.
        ...
        self.assertEqual(len(self.scrooge), 0)  # Not enqueued again.
        self.assertEqual(self.scrooge.result_count(), 0)
    ...
    def test_retries_with_override(self):
        ...
        self.assertEqual(len(self.scrooge), 1)
        self.assertEqual(self.scrooge.result_count(), 0)
        self.scrooge.dequeue()  # Throw away task.
    ...
    def test_retry_periodic(self):
        ...
        @self.scrooge.periodic_task(crontab(hour="0"), retries=2)
        def task_p():
            if state[0] == 0:
                state[0] = 1
    ...
    def test_retry_delay(self):
        @self.scrooge.task(retries=2, retry_delay=60)
        def task_a():
            raise ValueError("try again")
        ...
        self.assertEqual(len(self.scrooge), 0)
        self.assertEqual(self.scrooge.scheduled_count(), 1)
        (task,) = self.scrooge.scheduled()  # Dequeue the delayed retry.
        self.assertTrue(task.eta is not None)
        self.assertEqual(task.retries, 1)
        self.assertFalse(self.scrooge.ready_to_run(task))

        dt = datetime.datetime.now() + datetime.timedelta(seconds=61)
        self.assertTrue(self.scrooge.ready_to_run(task, dt))

    def test_retry_delay_periodic(self):
        @self.scrooge.periodic_task(crontab(), retries=2, retry_delay=60)
        def task_p():
            raise ValueError("try again")
    ...
    def test_retrytask_eta_delay(self):
        @self.scrooge.task(retry_delay=10)
        def task_a(d=None, e=None):
            raise RetryTask(delay=d, eta=e)

        seconds = lambda s: (datetime.datetime.now() + datetime.timedelta(seconds=s))
        ...
        def run_iteration(d, e):
            ...
            self.assertEqual(len(self.scrooge), 0)
            self.assertEqual(len(self.scrooge.scheduled()), 1)

            (task,) = self.scrooge.scheduled()
            self.assertEqual(task.id, r.id)
            self.assertFalse(self.scrooge.ready_to_run(task))
            if e is not None:
    ...
    def test_retrytask_explicit(self):
        ...
        self.assertEqual(self.scrooge.execute(task), 2)
        r.reset()
        self.assertEqual(r.get(), 2)
        self.assertEqual(state, [2])
        self.assertEqual(len(self.scrooge), 0)
    ...
    def test_read_schedule(self):
        ...
        for _ in range(3):
            self.scrooge.execute(self.scrooge.dequeue(), timestamp=server_time)

        self.assertEqual(self.scrooge.scheduled_count(), 3)
        self.assertEqual(len(self.scrooge), 0)

        # Two tasks are ready to run. Reading from the schedule is a
        # destructive operation.
        tn1, t0 = self.scrooge.read_schedule(timestamp)
        self.assertEqual(tn1.id, rn1.id)
        self.assertEqual(t0.id, r0.id)
        self.assertEqual(self.scrooge.read_schedule(timestamp), [])

        (tp1,) = self.scrooge.read_schedule(datetime.datetime(2010, 1, 1))
    ...
    def test_read_periodic(self):
        @self.scrooge.periodic_task(crontab(minute="*/15", hour="9-17"))
        def work():
            pass

        @self.scrooge.periodic_task(crontab(minute="0", hour="21"))
        def sleep():
            pass

        @self.scrooge.periodic_task(crontab(minute="0-30"))
        def first_half():
            pass

        def assertPeriodic(hour, minute, names):
            dt = datetime.datetime(2000, 1, 1, hour, minute)
            tasks = self.scrooge.read_periodic(dt)
...
class TestDecorators(BaseTestCase):
    ...
    def test_task_parameters(self):
        @self.scrooge.task(retries=2, retry_delay=1, context=True, name="tb")
    ...
    def test_periodic_task(self):
        @self.scrooge.periodic_task(crontab(minute="1"))
        def task_p():
            return 123
        ...
        @self.scrooge.periodic_task(crontab(), retries=3, retry_delay=10)
        def task_p2():
            pass
    ...
    def test_dynamic_periodic_tasks(self):
        ...
        def make_ptask(every_n):
            ...
            self.scrooge.periodic_task(sched, name=name)(ptask)
        ...
        def assertPeriodic(dt, names):
            ptasks = self.scrooge.read_periodic(dt)
...
class TestTaskHooks(BaseTestCase):
    def test_task_hooks(self):
        ...
        self.assertEqual(
            post_state, [(result.id, 4, None), (err_result.id, None, "TypeError")]
        )
    ...
    def test_hook_unregister(self):
        ...
        self.assertTrue(self.scrooge.unregister_pre_execute("test_pre_exec"))
        self.assertTrue(self.scrooge.unregister_post_execute("test_post_exec"))
        ...
        self.assertFalse(self.scrooge.unregister_pre_execute("test_pre_exec"))
        self.assertFalse(self.scrooge.unregister_post_execute("test_post_exec"))
...
class TestTaskChaining(BaseTestCase):
    ...
    def assertPipe(self, pipeline, expected):
        results = self.scrooge.enqueue(pipeline)
    ...
    def test_error_callback(self):
        ...
        result = self.scrooge.enqueue(task)
        self.assertTrue(self.execute_next() is None)
    ...
    def test_pipeline_error_midway(self):
        ...
        r1, r2, r3, r4 = self.scrooge.enqueue(pipe)
    ...
    def test_chain_errors(self):
        ...
        r1, r2 = self.scrooge.enqueue(pipe)
...
class TestTaskLocking(BaseTestCase):
    def test_task_locking(self):
        ...
        task_a(4)
        with self.scrooge.lock_task("lock_x"):
            self.assertFalse(self.scrooge.is_locked("lock_a"))
            self.assertTrue(self.scrooge.is_locked("lock_x"))
            self.assertEqual(self.execute_next(), 5)

        # Ensure locks were cleared.
        self.assertFalse(self.scrooge.is_locked("lock_a"))
        self.assertFalse(self.scrooge.is_locked("lock_x"))

        r = task_a(5)
        with self.scrooge.lock_task("lock_a"):
            self.assertTrue(self.scrooge.is_locked("lock_a"))
            self.assertTrue(self.execute_next() is None)
...
class TestScroogeAPIs(BaseTestCase):
    def test_flush_locks(self):
        with self.scrooge.lock_task("lock1"):
            with self.scrooge.lock_task("lock2"):
                flushed = self.scrooge.flush_locks()

        self.assertEqual(flushed, set(["lock1", "lock2"]))
        self.assertEqual(self.scrooge.flush_locks(), set())

    def test_flush_named_locks(self):
        self.scrooge.put_if_empty("%s.lock.lock1" % self.scrooge.name, "1")
        self.scrooge.put_if_empty("%s.lock.lock2" % self.scrooge.name, "1")
        with self.scrooge.lock_task("lock3"):
            flushed = self.scrooge.flush_locks("lock1", "lock2", "lockx")
    ...
    def test_serialize_deserialize(self):
        ...
        tb = task_b.s(2)
        tp = task_p.s()
        S = lambda t: self.scrooge.deserialize_task(self.scrooge.serialize_task(t))
        self.assertEqual(ta, S(ta))
        self.assertEqual(tb, S(tb))
        self.assertEqual(tp, S(tp))

    def test_serialize_deserialize_signed(self):
        self.scrooge.serializer = SignedSerializer(secret="test secret")
    ...
    def test_put_get(self):
        ...
        for test in tests:
            self.scrooge.put("key", test)
            self.assertEqual(self.scrooge.get("key"), test)

    def test_unsupported(self):
        FooScrooge = _unsupported("FooScrooge", "foo")
        self.assertRaises(ConfigurationError, FooScrooge)


class TestMultipleScrooge(BaseTestCase):
    def test_multiple_scrooge(self):
        scrooge1 = self.scrooge
        scrooge2 = MemoryScrooge("scrooge2", utc=False)

        @scrooge1.task()
        def task_a(n):
            return n + 1

        task_a2 = scrooge2.task(retries=1)(task_a)

        r = task_a(1)
        self.assertEqual(len(scrooge1), 1)
        self.assertEqual(len(scrooge2), 0)
        self.assertEqual(self.execute_next(), 2)
        self.assertEqual(r.get(), 2)

        r2 = task_a2(2)
        self.assertEqual(len(scrooge1), 0)
        self.assertEqual(len(scrooge2), 1)
        self.assertEqual(scrooge2.execute(scrooge2.dequeue()), 3)
        self.assertEqual(r2.get(), 3)


class TestDisableResultStore(BaseTestCase):
    def get_scrooge(self):
        return MemoryScrooge(results=False, utc=False)
    ...
    def test_disable_result_store(self):
        ...
        res = self.scrooge.enqueue(p)
        self.assertTrue(res is None)
        self.assertEqual(self.execute_next(), 4)
        self.assertEqual(self.execute_next(), 5)
        self.assertEqual(state, [2, 3, 4])

        self.scrooge.immediate = True
...
class TestStorageWrappers(BaseTestCase):
    def test_storage_wrappers(self):
        ...
        assertTasks(self.scrooge.pending(), [])
        assertTasks(self.scrooge.scheduled(), [])
        self.assertEqual(self.scrooge.all_results(), {})

        r1 = task_a(1)
        r2 = task_a.schedule((2,), delay=60)
        r3 = task_a(3)

        assertTasks(self.scrooge.pending(), [r1.id, r2.id, r3.id])
        assertTasks(self.scrooge.scheduled(), [])
        self.assertEqual(self.scrooge.all_results(), {})
        ...
        assertTasks(self.scrooge.pending(), [r3.id])
        assertTasks(self.scrooge.scheduled(), [r2.id])
        self.assertEqual(list(self.scrooge.all_results()), [r1.id])
...
class TestDocstring(BaseTestCase):
    def test_docstring_preserved(self):
        ...
        @self.scrooge.periodic_task(crontab(minute="*"))
        def ptask():
...
class TestCustomErrorMetadata(BaseTestCase):
    def get_scrooge(self):
        class CustomError(MemoryScrooge):
    ...
    def test_custom_error(self):
        ...
        re = task_e(0)
        self.assertTrue(self.execute_next() is None)
        self.assertEqual(self.scrooge.result_count(), 1)  # Error result present.
