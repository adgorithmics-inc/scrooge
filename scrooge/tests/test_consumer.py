...
from scrooge.api import crontab
from scrooge.consumer import Consumer, Scheduler
from scrooge.consumer_options import ConsumerConfig
...
class TestConsumerIntegration(BaseTestCase):
    ...
    def work_on_tasks(self, consumer, n=1, now=None):
        ...
        for i in range(n):
            self.assertEqual(len(self.scrooge), n - i)
            worker.loop(now)
    ...
    def test_consumer_schedule_task(self):
        ...
        self.assertEqual(self.scrooge.scheduled_count(), 2)
    ...
    def test_consumer_periodic_tasks(self):
        ...
        @self.scrooge.periodic_task(crontab(minute="*/10"))
        def task_p1():
            state.append("p1")

        @self.scrooge.periodic_task(crontab(minute="0", hour="0"))
        def task_p2():
            state.append("p2")
        ...
        task_p1.revoke()
        self.schedule_tasks(consumer, dt)
        self.assertEqual(len(self.scrooge), 1)  # Enqueued despite being revoked.
        self.work_on_tasks(consumer, 1)
...
class TestConsumerConfig(BaseTestCase):
    def test_default_config(self):
        cfg = ConsumerConfig()
        cfg.validate()
        consumer = self.scrooge.create_consumer(**cfg.values)
