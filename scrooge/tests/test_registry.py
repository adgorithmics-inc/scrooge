...
from scrooge.exceptions import ScroogeException
...
class TestRegistry(BaseTestCase):
    def setUp(self):
        ...
        self.registry = self.scrooge._registry
    ...
    def test_register_unique(self):
        ...
        ta = self.scrooge.task()(task_a)
        self.assertRaises(ValueError, self.scrooge.task(), task_a)
        self.assertRaises(ValueError, self.scrooge.task(name="task_a"), task_b)

        # We can register task_b and re-register task_a providing a new name.
        tb = self.scrooge.task()(task_b)
        ta2 = self.scrooge.task(name="task_a2")(task_a)
    ...
    def test_missing_task(self):
        ...
        self.assertTrue(task_a.unregister())
        self.assertRaises(ScroogeException, self.registry.create_task, message)

        # Similarly, we can no longer serialize the task to a message.
        self.assertRaises(ScroogeException, self.registry.create_message, task)

    def test_periodic_tasks(self):
        def task_fn():
            pass

        self.scrooge.task(name="a")(task_fn)
        p1 = self.scrooge.periodic_task(lambda _: False, name="p1")(task_fn)
        p2 = self.scrooge.periodic_task(lambda _: False, name="p2")(task_fn)
        self.scrooge.task(name="b")(task_fn)
    ...
    def test_scrooge1_compat(self):
        ...
        self.scrooge.storage.enqueue(pickle.dumps(old_message))

        self.assertEqual(len(self.scrooge), 1)
        self.assertEqual(self.execute_next(), 3)
        self.assertEqual(self.scrooge.result(t.id), 3)
