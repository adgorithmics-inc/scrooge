...
from scrooge.exceptions import ConfigurationError
from scrooge.storage import FileStorage, MemoryStorage, RedisExpireStorage
from scrooge.tests.base import TRAVIS, BaseTestCase
...
class TestRedisStorage(StorageTests, BaseTestCase):
    ...
    def test_conflicting_init_args(self):
        ...
        for kwargs in (dict(item) for item in combinations):
            self.assertRaises(ConfigurationError, lambda: RedisScrooge(**kwargs))

        # None values are fine, however.
        RedisScrooge(host=None, port=None, db=None, url="redis://localhost")
...
class TestRedisExpireStorage(StorageTests, BaseTestCase):
    ...
    def get_scrooge(self):
        return RedisExpireScrooge(expire_time=3600, utc=False, blocking=False)
    ...
    def test_expire_results(self):
        ...
        self.assertEqual(
            self.s.result_items(), {b"k1": b"v1", b"k3": b"v3", b"k4": b"v4"}
        )
    ...
    def test_integration_2(self):
        ...
        self.assertEqual(self.scrooge.result_count(), 1)  # Revoke key.

        self.assertEqual(self.execute_next(), 2)
        self.assertEqual(self.scrooge.result_count(), 2)  # Revoke key and r1.

        self.assertTrue(self.execute_next() is None)
        self.assertEqual(self.scrooge.result_count(), 1)  # Just r1 now.
        self.assertFalse(r2.is_revoked())

        self.assertEqual(self.execute_next(), 4)
        self.assertEqual(self.scrooge.result_count(), 2)  # r1 and r3.
        ...
        for _ in range(3):
            ...
            r1.reset()
            r3.reset()
        self.assertEqual(self.scrooge.result_count(), 2)  # r1 and r3 still there.
...
class TestPriorityRedisStorage(TestRedisStorage):
    def get_scrooge(self):
        return PriorityRedisScrooge(utc=False)


@unittest.skipIf(get_redis_version() < 5, "Requires Redis >= 5.0")
class TestPriorityRedisStorageNotBlocking(TestRedisStorage):
    def get_scrooge(self):
        return PriorityRedisScrooge(utc=False, blocking=False)


class TestSqliteStorage(StorageTests, BaseTestCase):
    def tearDown(self):
        super(TestSqliteStorage, self).tearDown()
        if os.path.exists("scrooge_storage.db"):
            os.unlink("scrooge_storage.db")

    def get_scrooge(self):
        return SqliteScrooge(filename="scrooge_storage.db", timeout=3)
...
class TestFileStorageMethods(StorageTests, BaseTestCase):
    path = "/tmp/test-scrooge-storage"
    result_path = "/tmp/test-scrooge-storage/results"
    queue_path = "/tmp/test-scrooge-storage/queue"
    ...
    def test_fs_multithreaded(self):
        ...
        def create_tasks(t, n, q):
            for i in range(n):
                with l:
                    message = str((t * n) + i)
                    self.scrooge.storage.enqueue(message.encode("utf8"))
                    q.put(message)

        def dequeue_tasks(q):
            while True:
                with l:
                    data = self.scrooge.storage.dequeue()
                    if data is None:
                        break
        ...
        self.assertEqual(self.scrooge.pending_count(), nthreads * ntasks)
        ...
        self.assertEqual(self.scrooge.pending_count(), 0)
